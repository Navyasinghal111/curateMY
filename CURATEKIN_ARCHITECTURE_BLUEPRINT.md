...
1. **Creator applies** at `/signup/creator` — a three-step flow (Apply → Social presence → Taste profile). Instagram is collected as a manually entered handle or profile link, not verified via OAuth — there's no mandatory "Connect Instagram" step. `instagram_verified` is set to `false` at signup every time; verification, when it happens, is manual and happens during admin review, not automatically.
2. **Real signup fires** — `supabase.auth.signUp()` is called with all collected fields riding along as auth `user_metadata` (since there's no session yet to write to `profiles` under RLS). This includes `brands_worked_with`, a field collected in the Taste profile step — but `profiles` has no matching column for it yet, so `/signup/confirm` intentionally does not write it into the database. It's collected, but not persisted anywhere durable today.
...
7. **Payout details are collected later, only when needed** — not at signup. This is a deliberate product decision made this session: both the creator signup form and `/signup/confirm`'s database insert hardcode `upi_id: null, pan_number: null` unconditionally — not read from metadata, not defaulted, just always `null` at this stage — so payout data can never enter the system at signup time regardless of what a stale or malformed metadata payload might contain. UPI/PAN collection happens post-approval, from Dashboard → Settings, right before a creator would actually need to be paid. Signup should ask for the minimum needed to evaluate someone's taste and reach, not everything the business might eventually need.
...
**Creator attribution must survive the wrap.** Whatever tracking parameter or sub-ID scheme Cuelinks uses needs to encode which creator (and ideally which product click) drove the traffic, so that when a commission comes back from Cuelinks, it can be matched back to the right creator's `creator_payout_details` for payout. Note that the product deliberately does not promise a fixed commission rate anywhere in its copy today — signup and marketing copy were rewritten this session specifically to avoid promising a number or a payout timeline before real tracking exists. Losing the attribution linkage would still be a real problem (a creator's traffic driving revenue with no way to credit them), just not one framed around a specific percentage — this is not a detail to defer casually.
...
- **Functional flow:** Three steps — Apply (identity + credentials), Social presence (Instagram/Pinterest/YouTube handles + reach — Instagram is a required field, but a manually typed handle or profile link, not an OAuth connection), Taste profile (niche, style, language). Each step's Next button stays disabled until that step is genuinely complete.
- **Technical flow:** Client component; real `auth.signUp()`. Instagram is a plain text input (`igHandle`), saved as `instagram_handle` — there is no OAuth "Connect Instagram" step in this flow, and `instagram_verified` is always written as `false` at signup, since verification is manual (an admin checks the account during review), not automated. An earlier version of this flow used real Instagram OAuth via `/api/auth/instagram`, with `sessionStorage` draft-persistence to survive the redirect — that mechanism was removed this session in favor of the simpler manual-entry approach, since OAuth reliability shouldn't be what stands between a creator and finishing their application.
...
The application flow (`/signup/creator`) was rebuilt this session to match the premium/editorial direction. Instagram is collected as a manually entered handle rather than through an OAuth connection — verification happens manually during admin review, not automatically — and payout collection is deliberately deferred out of signup entirely (`upi_id`/`pan_number` hardcoded `null` throughout the signup path). `/admin` has also since been rebuilt to the same premium design standard, with an explicit safe-column query that never fetches PAN/UPI. Still remaining: add `admin_notes`, and move the admin-access check from a hardcoded client-side email list toward something RLS can enforce directly as the team grows.
