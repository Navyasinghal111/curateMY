...
const inpFocus = { borderColor:'#0A0A0A' }

function Input({ type='text', placeholder, value, onChange, required=false, minLength, maxLength, disabled }:
  { type?:string; placeholder:string; value:string; onChange:(v:string)=>void; required?:boolean; minLength?:number; maxLength?:number; disabled?:boolean }) {
  ...
  return (
    <input
      ...
      style={{ ...inp, ...(focused ? inpFocus : {}), opacity: disabled ? 0.4 : 1 }}
...
function Select({ value, onChange, children }: { value:string; onChange:(v:string)=>void; children:React.ReactNode }) {
  ...
  return (
    <select
      ...
      style={{ ...inp, ...(focused ? inpFocus : {}), appearance:'none', cursor:'pointer' }}
      onFocus={() => setFocused(true)}
      onBlur={() => setFocused(false)}>
      {children}
    </select>
  ...
  return (
    <label style={{ display:'flex', alignItems:'flex-start', gap:12, cursor:'pointer' }}>
      <div onClick={onChange} style={{
        width:18, height:18, border:`1.5px solid ${checked ? '#0A0A0A' : '#D4D4D4'}`,
        borderRadius:4, flexShrink:0, marginTop:2,
        display:'flex', alignItems:'center', justifyContent:'center',
        background: checked ? '#0A0A0A' : '#fff', cursor:'pointer', transition:'all 0.15s'
      }}>
...
const STEPS = ['Basic info','Your platforms','Your content','Instagram','Payouts']
...
export default function CreatorSignupPage() {
  ...
  if (done) return (
    <div style={{ minHeight:'100vh', background:'#fff', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'DM Sans, sans-serif' }}>
      ...
      <p style={{ fontSize:10, letterSpacing:'0.2em', color:'#9B9B9B', marginBottom:12, textTransform:'uppercase' }}>Application received</p>
      <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:36, fontWeight:300, color:'#0A0A0A', marginBottom:16, textAlign:'center' }}>You're on your way.</h1>
  ...
  const NavButtons = ({ onNext, nextLabel='NEXT', isSubmit=false, showBack=true }: { onNext:()=>void; nextLabel?:string; isSubmit?:boolean; showBack?:boolean }) => (
  ...
  const Label = ({ children }: { children: React.ReactNode }) => (
    <p style={{ fontSize:11, letterSpacing:'0.1em', color:'#9B9B9B', textTransform:'uppercase', marginBottom:6 }}>{children}</p>
  )
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#fff', fontFamily:'DM Sans, sans-serif' }}>
        ...
        <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 48px', height:60, borderBottom:'0.5px solid #F0F0F0', position:'sticky', top:0, background:'#fff', zIndex:10 }}>
          ...
          <Link href="/login" style={{ fontSize:13, color:'#6B6B6B', textDecoration:'none' }}>Already have an account? <span style={{ color:'#0A0A0A', textDecoration:'underline' }}>Log in</span></Link>
        </nav>
        ...
        <div style={{ maxWidth:560, margin:'0 auto', padding:'56px 24px 80px' }}>
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <div key={s} style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  {i < STEPS.length - 1 && (
                    <div style={{ width:32, height:1, background: step > n ? '#0A0A0A' : '#E5E5E5' }} />
                  )}
                </div>
              )
            })}
          ...
          {step === 1 && (
            <div>
              <p style={{ fontSize:11, letterSpacing:'0.14em', color:'#9B9B9B', textTransform:'uppercase', textAlign:'center', marginBottom:12 }}>Apply to be a</p>
              <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:clamp(40,56), fontWeight:300, color:'#0A0A0A', textAlign:'center', lineHeight:1.1, marginBottom:12 }}>
                CurateKin Creator
              </h1>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <Input type="password" placeholder="Password (min 6 characters)*" value={password} onChange={setPass} required minLength={6} />
                <Input placeholder="City*" value={city} onChange={setCity} required />
                <div style={{ display:'flex', gap:10 }}>
                  <select value={countryCode} onChange={e => setCountryCode(e.target.value)} style={{ ...inp, width:90, flexShrink:0, appearance:'none' }}>
                    {['+91','+1','+44','+971','+65'].map(c => <option key={c}>{c}</option>)}
                  </select>
                  <Input type="tel" placeholder="Phone number* (10 digits)" value={phone} onChange={v => setPhone(v.replace(/\D/g,''))} maxLength={10} />
                </div>
                ...
                <p style={{ textAlign:'center', fontSize:12, color:'#9B9B9B', marginTop:4 }}>
                  Are you a brand?{' '}
                  <Link href="/signup" style={{ color:'#0A0A0A', textDecoration:'underline' }}>Apply here</Link>
                </p>
                {/* Editor shortcut */}
                <a href="/dashboard" style={{ display:'block', textAlign:'center', fontSize:11, color:'#D4D4D4', letterSpacing:'0.06em', padding:'8px', border:'1px dashed #E5E5E5', textDecoration:'none', borderRadius:4 }}>
          ...
          {step === 2 && (
            <div>
              <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:40, fontWeight:300, color:'#0A0A0A', textAlign:'center', lineHeight:1.1, marginBottom:8 }}>Your platforms</h1>
              <p style={{ fontSize:14, color:'#6B6B6B', textAlign:'center', marginBottom:40 }}>It's about taste, not follower count.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <Label>Primary platform</Label>
                <Select value={primaryPlatform} onChange={setPrimaryPlatform}>
                  <option value="">Select platform*</option>
                  {PLATFORMS.map(p => <option key={p}>{p}</option>)}
                </Select>
                <Input placeholder="Handle or profile URL*" value={primaryHandle} onChange={setPrimaryHandle} />
                <Select value={primaryFollowers} onChange={setPrimaryFollowers}>
                  <option value="">Follower count*</option>
                  {FOLLOWERS.map(r => <option key={r}>{r}</option>)}
                </Select>
                <Select value={engagementRate} onChange={setEngagementRate}>
                ...
                <div style={{ borderTop:'1px solid #F0F0F0', paddingTop:20, marginTop:4 }}>
                  <Label>Secondary platform (optional)</Label>
                  <div style={{ display:'flex', flexDirection:'column', gap:10, marginTop:6 }}>
                    <Select value={secondaryPlatform} onChange={setSecondaryPlatform}>
                      <option value="">Select platform</option>
                      {PLATFORMS.map(p => <option key={p}>{p}</option>)}
                    </Select>
                    {secondaryPlatform && <>
                      <Input placeholder="Handle or profile URL" value={secondaryHandle} onChange={setSecondaryHandle} />
                      <Select value={secondaryFollowers} onChange={setSecondaryFollowers}>
                        <option value="">Follower count</option>
                        {FOLLOWERS.map(r => <option key={r}>{r}</option>)}
                      </Select>
                    </>}
          ...
          {step === 3 && (
            <div>
              <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:40, fontWeight:300, color:'#0A0A0A', textAlign:'center', lineHeight:1.1, marginBottom:8 }}>Your content</h1>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div>
                  <Label>Bio & content style*</Label>
                  <textarea
                    placeholder="Describe your content style, aesthetic, and what makes your recommendations unique..."
                ...
                <Input placeholder="Referral code (optional)" value={referral} onChange={setReferral} />
                <NavButtons onNext={() => {
          ...
          {step === 4 && (
            <div>
              <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:40, fontWeight:300, color:'#0A0A0A', textAlign:'center', lineHeight:1.1, marginBottom:8 }}>Verify Instagram</h1>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                {igConnected
                  ...
                  : <button onClick={connectInstagram} style={{
                ...
                <NavButtons onNext={next} nextLabel={igConnected ? 'NEXT' : 'SKIP FOR NOW →'} />
                {!igConnected && (
          ...
          {step === 5 && (
            <div>
              <h1 style={{ fontFamily:'Cormorant Garamond, serif', fontSize:40, fontWeight:300, color:'#0A0A0A', textAlign:'center', lineHeight:1.1, marginBottom:8 }}>Payouts & agreement</h1>
              <p style={{ fontSize:14, color:'#6B6B6B', textAlign:'center', marginBottom:40 }}>Almost there. Set up how you'll get paid.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                <div>
                  <Label>UPI ID</Label>
                  <Input placeholder="e.g. yourname@okicici*" value={upiId} onChange={setUpiId} />
                ...
                <div>
                  ...
                  <Input placeholder="e.g. ABCDE1234F*" value={panNumber} onChange={v => setPanNumber(v.toUpperCase())} maxLength={10} />
                ...
                <div style={{ borderTop:'1px solid #F0F0F0', paddingTop:20, display:'flex', flexDirection:'column', gap:14 }}>
                  ...
                  <Checkbox checked={agreedTos} onChange={() => setAgreedTos(a => !a)}>
                    I agree to the{' '}
                    <a href="/terms" target="_blank" style={{ color:'#0A0A0A', textDecoration:'underline' }}>Terms & Conditions</a>
                    {' '}and{' '}
                  ...
                  <Checkbox checked={agreedAffiliate} onChange={() => setAgreedAffiliate(a => !a)}>
                    I agree to the{' '}
                    <a href="/affiliate-policy" target="_blank" style={{ color:'#0A0A0A', textDecoration:'underline' }}>Affiliate & Commission Policy</a>,
                    including the 80% commission structure and monthly payout schedule*
                  </Checkbox>
                </div>
                <NavButtons onNext={() => submit(false)} nextLabel="SUBMIT APPLICATION" isSubmit />
